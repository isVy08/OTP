...
model = GMM(D, K, tau, a=X[0, ].min().item(), b=X[0, ].max().item())
...
for iter in range(num_iters):
    ...
    z, _  = phi(X, 1)        
    Xhat = model(z)
    loss = compute_loss(X, Xhat, Z, Zhat, 1.0, metric, logZ, logZhat)
    bopt.zero_grad()
    loss.backward()
    bopt.step()
    ...
    z, _  = phi(X, 1)        
    Xhat = model(z)
    obj = compute_loss(X, Xhat, Z, Zhat, 1.0, metric, logZ, logZhat)
    fopt.zero_grad()
    obj.backward()
    fopt.step()
